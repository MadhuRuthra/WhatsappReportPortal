...
async function ManualUploadList(req) {
    try {
        ...
        manual_list = `SELECT DISTINCT cwt.campaign_name, cwt.campaign_id, cwt.total_updated_count, cwt.total_mobileno_count, cwt.whatsapp_status,cwt.remarks, cwt.compose_whatsapp_id,cwt.whatsapp_entry_date, cwt.user_id, usr.user_name,tgbase_count  AS count_pj,cgbase_count AS count_yj,cwt.upload_info FROM master_compose_whatsapp cwt LEFT JOIN whatsapp_report.user_management usr ON cwt.user_id = usr.user_id WHERE cwt.whatsapp_status in ('P','V') group by campaign_id ORDER BY whatsapp_entry_date DESC`;
