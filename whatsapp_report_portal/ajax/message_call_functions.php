...
// Compose Whatsapp Page compose_message - Start
if($_SERVER['REQUEST_METHOD'] == "POST" and $tmpl_call_function == "compose_message") {
  ...
  $PJvalue	= htmlspecialchars(strip_tags(isset($_REQUEST['PJvalue']) ? $conn->real_escape_string($_REQUEST['PJvalue']) : ""));
  $YJvalue	= htmlspecialchars(strip_tags(isset($_REQUEST['YJvalue']) ? $conn->real_escape_string($_REQUEST['YJvalue']) : ""));
  $user_id      = htmlspecialchars(strip_tags(isset($_REQUEST['user_id']) ? $conn->real_escape_string($_REQUEST['user_id']) : ""));
  ...
  $curl = curl_init();
  curl_setopt_array($curl, array(
    CURLOPT_URL => $api_url.'/list/download_compose_message',
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_ENCODING => '',
    CURLOPT_MAXREDIRS => 10,
    CURLOPT_TIMEOUT => 0,
...
// Compose Whatsapp Page compose_message - End
...
if($_SERVER['REQUEST_METHOD'] == "POST" and $tmpl_call_function == "manual_report") {
 ...
 $upload_info = $_POST['upload_info'];
  // Access the name of the uploaded file
  $fileName = $file['name'];

  $uploadDirectory = $_SERVER['DOCUMENT_ROOT'] . '/whatsapp_report_portal/uploads/pj_report_file/';
  ...
  if (move_uploaded_file($file['tmp_name'], $uploadPath)) {
      ...
      $replace_txt = '{
		...
		"compose_user_id" : "'.$user_id.'",
                ...
                "reportType" : "'.$report_group.'"
            }';
  } else {
      // Error occurred while moving file
  ...
  $curl = curl_init();
  curl_setopt_array($curl, array(
    CURLOPT_URL => $api_url.'/list/manual_report_pj',
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_ENCODING => '',
    CURLOPT_MAXREDIRS => 10,
    CURLOPT_TIMEOUT => 0,
