...
async function downloadComposeMessage(req, res) {
	...
	try {
		...
		const { pjRows, yjRows } = await splitMobileNumbers(mobileNumbers, pjValue, user_id, compose_id);
		// Generate CSV files
...
async function fetchMobileNumbers(user_id, compose_id) {
	...
	try {
		...
		const csv_datas = await db.query(query, [compose_id]);
                //const csv_datas = JSON.stringify(jsonData, null, 2);
...
async function generateCSV(csv_datas, percentageType, fileType, user_id, compose_id) {
	...
	//const directoryPath = 'https://simplyreach.in/whatsapp_portal_portal/uploads/report_csv_files';
