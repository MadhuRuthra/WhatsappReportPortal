...
async function Filter_Campaign_Name(req) {
  ...
  if(filter_department){
    ...
    function dateRange(startDate, endDate, steps = 1) {
      ...
      while (currentDate <= new Date(endDate)) {
        ...
        for (i = 0; i < array_list_user_id.length; i++) {
          get_campaign_name += ` SELECT  wht.compose_whatsapp_id, wht.store_id, wht.campaign_name, wht.message_type, wht.total_mobileno_count,DATE_FORMAT(wht.whatsapp_entry_date,'%d-%m-%Y %h:%i:%s %p')  FROM whatsapp_report_${array_list_user_id[i]}.compose_whatsapp_tmpl_${array_list_user_id[i]} wht where (date(wht.whatsapp_entry_date) BETWEEN '${slt_date}' and '${slt_date}') UNION`;
        }
    ...
    var get_campaign_name_1 = await dynamic_db.query(get_campaign_name1, null, `whatsapp_report_${user_id}`);
...
async function ReportCampaignName(req) {
  try {
    ...
    for (i = 0; i < array_list_user_id.length; i++) {
        get_campaign_name += ` SELECT * FROM whatsapp_report_${array_list_user_id[i]}.compose_whatsapp_tmpl_${array_list_user_id[i]} wht where wht.campaign_name in ('${campaign_name_filer}') UNION`;
    }
    ...
    function dateRange(startDate, endDate, steps = 1) {
      ...
      while (currentDate <= new Date(endDate)) {
        ...
        // loop all the array_list_user_id.
        for (i = 0; i < array_list_user_id.length; i++) {
          get_campaign_report += ` SELECT wht.user_id,wht.campaign_name,wht.store_id, usr.user_name,ussr.user_type,ussr.user_type,ml.available_messages,DATE_FORMAT(wht.whatsapp_entry_date,'%d-%m-%Y') entry_date, count(stt.comwtap_status_id) total_msg,(select count(distinct comwtap_status_id) from whatsapp_report_${array_list_user_id[i]}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} where response_status = 'S' and compose_whatsapp_id in ('${comp_wastp_id}') ) total_success,(select count(distinct comwtap_status_id) from whatsapp_report_${array_list_user_id[i]}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} where  response_status = 'S' and delivery_status = 'Y' and compose_whatsapp_id in ('${comp_wastp_id}') ) total_delivered,(select count(distinct comwtap_status_id) from whatsapp_report_${array_list_user_id[i]}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} where response_status = 'S' and read_status = 'Y' and compose_whatsapp_id in ('${comp_wastp_id}')) total_read,(select count(distinct comwtap_status_id) from whatsapp_report_${array_list_user_id[i]}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} where response_status = 'F' and compose_whatsapp_id in ('${comp_wastp_id}')) total_failed,(select count(distinct comwtap_status_id) from whatsapp_report_${array_list_user_id[i]}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} where response_status = 'I' and compose_whatsapp_id in ('${comp_wastp_id}')) total_invalid,(select count(distinct comwtap_status_id) from whatsapp_report_${array_list_user_id[i]}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} where (response_status not in ('I', 'F', 'S') or response_status is null) and compose_whatsapp_id in ('${comp_wastp_id}')) total_waiting FROM whatsapp_report_${array_list_user_id[i]}.compose_whatsapp_tmpl_${array_list_user_id[i]} wht left join whatsapp_report_${array_list_user_id[i]}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} stt on wht.compose_whatsapp_id = stt.compose_whatsapp_id left join whatsapp_report.user_management usr on wht.user_id = usr.user_id left join whatsapp_report.message_limit ml on wht.user_id = ml.user_id left join whatsapp_report.user_master ussr on usr.user_master_id = ussr.user_master_id where usr.user_id = '${array_list_user_id[i]}' and wht.compose_whatsapp_id in ('${comp_wastp_id}') and (date(wht.whatsapp_entry_date) BETWEEN '${slt_date}' and '${slt_date}')   UNION`;
        }
        currentDate.setUTCDate(currentDate.getUTCDate() + steps);
      }
      return dateArray;
    ...
    const dates = dateRange(filter_date_1[0], filter_date_1[1]);
      ...
      var get_campaign_name_1 = await dynamic_db.query(get_campaign_report_1, null, `whatsapp_report_${user_id}`);
