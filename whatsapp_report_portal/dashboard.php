...
<body>
  ...
  <script>
    ...
    var user_name, available_messages, total_msg, total_success, total_delivered, total_waiting,tot_failed,tot_read;
    var available_messagesArray, user_nameArray, total_successArray, total_deliveredArray, total_waitingArray,tot_failedArray, tot_readArray;
  ...
  $.ajax({
	...
	var total_delivered = document.getElementById('total_delivered').value;
        var total_waiting = document.getElementById('total_waiting').value;
        var tot_failed = document.getElementById('tot_failed').value;
   //var tot_read = document.getElementById('tot_read').value;
	...
	var total_deliveredArray = JSON.parse(total_delivered);
        var total_waitingArray = JSON.parse(total_waiting);
        var tot_failedArray = JSON.parse(tot_failed);
         //var tot_readArray = JSON.parse(tot_read);

        // var user_nameArrayUpperCase = user_name.map(name => name.toUpperCase());
	...
	total_deliveredArray = total_deliveredArray.map(function (value) {
          return Number(value);
        });

        //total_readArray = total_readArray.map(function (value) {
          //return Number(value);
        //});
  ...
  scales: {
    xAxes: [{
      ticks: {
        ...
        maxRotation: 90, // Rotate labels if necessary for better readability
        minRotation: 0  // Reset minimum rotation angle
      }
    }],
    yAxes: [{
      ticks: {
