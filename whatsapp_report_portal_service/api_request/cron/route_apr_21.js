...
async function cronfolder() {
  ...
  try {
    ...
    var update_queue = `Update queue_process set status = 'P' where compose_message_id = '${compose_id}'`;
    ...
    async function file_processing() {
      ...
      if (invalid_date) {
        var update_queue = `Update queue_process set remarks = '${invalid_date}' where status = 'P' and compose_message_id = ${compose_id}`;
        var update_queue_result = await db.query(update_queue);
        logger_all.info(JSON.stringify(update_queue_result));
      }
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (i % batchSize === 0) {
          ...
          // logger_all.info(JSON.stringify(update_report_status));update_query
      ...
      var update_queue = `Update queue_process set status = 'C' where compose_message_id = '${compose_id}'`;
      var update_queue_result = await db.query(update_queue);
      logger_all.info(JSON.stringify(update_queue_result));
