...
async function approveComposeMessage(req) {
    ...
    try {
		...
		//var approve_message = "SELECT * FROM whatsapp_report_" + user_id + " . compose_whatsapp_tmpl_" + user_id + " WHERE whatsapp_status = 'W'";

      //  var approve_message = "SELECT c.*, u.user_name FROM whatsapp_report_" + user_id + " . compose_whatsapp_tmpl_" + user_id + " AS c JOIN whatsapp_report.user_management AS u ON c.user_id = u.user_id WHERE c.whatsapp_status = 'W'";
        ...
        for (let i = 0; i < userIds.length; i++) {
            ...
            // Construct the SQL query to fetch approval details for the current user ID
             approve_message = approve_message+`
                SELECT c.campaign_name, c.total_mobileno_count, c.message_type, c.content_message_count,c.tg_base,c.cg_base,DATE_FORMAT(c.whatsapp_entry_date,"%d-%m-%Y %H:%i:%s") whatsapp_entry_date, c.whatsapp_status, c.user_id, c.compose_whatsapp_id, c.total_mobileno_count,  u.user_name 
                ...
                WHERE c.whatsapp_status in ('P','W','S','V') UNION `;

            // Process the approval details for the current user ID
            console.log(`Approval details for user ID ${userId}:`, approve_message);
        }
        approve_message = approve_message.slice(0,-7)
        //var approve_message = "SELECT c.*, u.user_name FROM whatsapp_report_" + user_id + " . compose_whatsapp_tmpl_" + user_id + " AS c JOIN whatsapp_report.user_management AS u ON c.user_id = u.user_id WHERE c.whatsapp_status = 'W'";

        logger_all.info("[select query request] : " + approve_message +  'ORDER BY STR_TO_DATE(whatsapp_entry_date, "%d-%m-%Y %H:%i:%s") DESC');
        const get_approve_whatsapp_no_api = await db.query(approve_message + 'ORDER BY STR_TO_DATE(whatsapp_entry_date, "%d-%m-%Y %H:%i:%s") DESC');
