  ...
  try {
    ...
    async function file_processing() {
      ...
      if (invalid_date) {
        ...
        logger_all.info("coming invalid date ");
      }

      if (invalid_date) {
        var update_queue = `Update master_compose_whatsapp set remarks = '${invalid_date}' where compose_whatsapp_id = '${compose_id}' and user_id = '${user_id}'`;
      ...
      if (select_compose[0].cgbase_count > 0) {
        ...
        console.log(`UPDATE whatsapp_report.master_compose_whatsapp SET updated_tgbase = '${select_compose[0].tgbase_count}',total_updated_count = '${totaltg_count}' WHERE compose_whatsapp_id = ? and user_id = ?`, [compose_id, user_id]);

        update_compose = await db.query(`UPDATE whatsapp_report.master_compose_whatsapp SET updated_tgbase = '${select_compose[0].tgbase_count}',total_updated_count = '${totaltg_count}' WHERE compose_whatsapp_id = ? and user_id = ?`, [compose_id, user_id]);

      } else {

        console.log(`UPDATE whatsapp_report.master_compose_whatsapp SET updated_tgbase = '${totaltg_count}',total_updated_count = '${totaltg_count}' WHERE compose_whatsapp_id = ? and user_id = ?`, [compose_id, user_id]);
