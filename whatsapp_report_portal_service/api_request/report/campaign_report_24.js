...
async function MobileReport(req) {
  try {
    ...
    get_campaign = await dynamic_db.query(`SELECT cam.compose_whatsapp_id,cam.campaign_name,cam.message_type,cam.total_mobileno_count,sts.mobile_no,sts.comments sender, DATE_FORMAT(sts.comwtap_entry_date,'%d-%m-%Y %h:%i:%s %p') comwtap_entry_date,sts.response_status,sts.response_message,sts.response_id, DATE_FORMAT(sts.response_date,'%d-%m-%Y %h:%i:%s %p') response_date,sts.delivery_status,sts.read_status, DATE_FORMAT(sts.delivery_date,'%d-%m-%Y %h:%i:%s %p') delivery_date, DATE_FORMAT(sts.read_date,'%d-%m-%Y %h:%i:%s %p') read_date FROM compose_whatsapp_status_tmpl_${user_id} sts
        LEFT JOIN compose_whatsapp_tmpl_${user_id} cam ON cam.compose_whatsapp_id = sts.compose_whatsapp_id
        WHERE sts.mobile_no = '${mobile_number}' order by compose_whatsapp_id desc`, null, `whatsapp_report_${user_id}`);
  ...
  try {
    ...
    if (mobile_number.length != 0) {
      ...
      for (var m = 0; m < mobile_number.length; m++) {
        ...
        var getdelivery_array = await dynamic_db.query(get_delivery_report, null, `whatsapp_report_${user_id}`);
...
async function OtpSummaryReport(req) {
 ...
 if (getsummary.length == 0) {
            var myObj = {
              ...
              "whatsapp_entry_date": today_date,
              ...
              "total_delivered": 0,
              "total_read": 0,
              "campaign_name":total_campaign_name[i]
            }
  ...
  if (!filter_date && !store_id_filter) {
        ...
        if (filter_date) {
  logger_all.info("FIlter date coming")
          ...
          function dateRange(startDate, endDate, steps = 1) {
            ...
            while (currentDate <= new Date(endDate)) {
              ...
              slt_date = convert(currentDate);
             		...
             		get_summary_report += ` SELECT wht.user_id, wht.store_id,wht.campaign_name, wht.campaign_id, usr.user_name, ussr.user_type,wht.whatsapp_entry_date, ml.available_messages, DATE_FORMAT(wht.whatsapp_entry_date,'%d-%m-%Y') AS entry_date, wht.total_mobileno_count AS total_msg, COUNT(DISTINCT CASE WHEN wht.whatsapp_status = 'S' AND stt.delivery_status = 'SENT' THEN stt.comwtap_status_id END) AS total_success,COUNT(DISTINCT CASE WHEN wht.whatsapp_status = 'S' AND stt.delivery_status = 'DELIVERED' THEN stt.comwtap_status_id END) AS total_delivered, COUNT(DISTINCT CASE WHEN wht.whatsapp_status = 'S' AND stt.delivery_status = 'READ' THEN stt.comwtap_status_id END) AS total_read,COUNT(DISTINCT CASE WHEN wht.whatsapp_status = 'S' AND stt.delivery_status IN ('BLOCKED', 'INCAPABLE', 'NOT AVAILABLE', 'FAILED','INVALID','UNAVAILABLE') THEN stt.comwtap_status_id END) AS total_failed, COUNT(DISTINCT CASE WHEN wht.whatsapp_status = 'S' AND stt.delivery_status = 'INVALID' THEN stt.comwtap_status_id END) AS total_invalid, COUNT(DISTINCT CASE WHEN wht.whatsapp_status = 'S' AND stt.delivery_status IS NULL THEN stt.comwtap_status_id END) AS total_waiting FROM  ${newdb}.compose_whatsapp_tmpl_${array_list_user_id[i]} wht LEFT JOIN ${newdb}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} stt ON wht.compose_whatsapp_id = stt.compose_whatsapp_id LEFT JOIN whatsapp_report.user_management usr ON wht.user_id = usr.user_id LEFT JOIN whatsapp_report.message_limit ml ON wht.user_id = ml.user_id LEFT JOIN  whatsapp_report.user_master ussr ON usr.user_master_id = ussr.user_master_id WHERE wht.whatsapp_status != 'R' and usr.user_id = '${array_list_user_id[i]}' and DATE(wht.whatsapp_entry_date) BETWEEN '${slt_date}' AND '${slt_date}' Group By wht.campaign_name union `; 

logger_all.info('[summary query request] : ' + get_summary_report);
      ...
      if (!store_id_filter) {
        ...
        var lastIndex = get_summary_report.lastIndexOf(" union ");
        ...
        getsummary = await dynamic_db.query(get_summary_report + ` order by whatsapp_entry_date desc `, null, `whatsapp_report_${user_id}`);
   }
...
return {
    ...
    return {
      response_code: 0, response_status: 201, response_msg: 'Error occured'+e
    };
  }
...
var user_id  = req.body.user_id;
  ...
  var query = await db.query(query_1 );
      ...
      function dateRange(startDate, endDate, steps = 1) {
        ...
        while (currentDate <= new Date(endDate)) {
          ...
          for (var i = 0; i < array_list_user_id.length; i++) {
            ...
            get_delivery_report += ` SELECT wht.compose_whatsapp_id,wht.whatsapp_content,wht.campaign_id,tmp.templateid, usr.user_name,wht.store_id, wht.campaign_name, wht.message_type, wht.total_mobileno_count, stt.mobile_no, stt.comments sender, DATE_FORMAT(stt.comwtap_entry_date,'%d-%m-%Y %h:%i:%s %p') comwtap_entry_date, stt.response_status, stt.response_message, stt.response_id,stt.response_date, DATE_FORMAT(stt.response_date,'%d-%m-%Y %h:%i:%s %p') response_dates, stt.delivery_status, stt.read_status, DATE_FORMAT(stt.delivery_date,'%d-%m-%Y %h:%i:%s %p') delivery_date, DATE_FORMAT(stt.read_date,'%d-%m-%Y %h:%i:%s %p') read_date FROM whatsapp_report_${array_list_user_id[i]}.	compose_whatsapp_tmpl_${array_list_user_id[i]} wht left join whatsapp_report_${array_list_user_id[i]}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} stt on wht.compose_whatsapp_id = stt.compose_whatsapp_id left join whatsapp_report.user_management usr on wht.user_id = usr.user_id   left join whatsapp_report.message_template tmp on tmp.unique_template_id = wht.unique_template_id where (wht.user_id = '${array_list_user_id[i]}' or usr.parent_id = '${array_list_user_id[i]}') and (date(stt.response_date) BETWEEN '${slt_date}' and '${slt_date}') ${filter} UNION`;
    ...
    // if the request read_date_filter is using the condition will be apply
      ...
      function dateRange(startDate, endDate, steps = 1) {
        ...
        while (currentDate <= new Date(endDate)) {
          ...
          for (var i = 0; i < array_list_user_id.length; i++) {
            ...
            get_delivery_report += ` SELECT wht.compose_whatsapp_id,wht.whatsapp_content, usr.user_name,wht.store_id, wht.campaign_name, wht.campaign_id,tmp.templateid,wht.message_type, wht.total_mobileno_count, stt.mobile_no, stt.comments sender, DATE_FORMAT(stt.comwtap_entry_date,'%d-%m-%Y %h:%i:%s %p') comwtap_entry_date, stt.response_status, stt.response_message, stt.response_id, DATE_FORMAT(stt.response_date,'%d-%m-%Y %h:%i:%s %p') response_dates,stt.response_date, stt.delivery_status, stt.read_status, DATE_FORMAT(stt.delivery_date,'%d-%m-%Y %h:%i:%s %p') delivery_date, DATE_FORMAT(stt.read_date,'%d-%m-%Y %h:%i:%s %p') read_date FROM whatsapp_report_${array_list_user_id[i]}.	compose_whatsapp_tmpl_${array_list_user_id[i]} wht left join whatsapp_report_${array_list_user_id[i]}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} stt on wht.compose_whatsapp_id = stt.compose_whatsapp_id left join whatsapp_report.user_management usr on wht.user_id = usr.user_id    left join whatsapp_report.message_template tmp on tmp.unique_template_id = wht.unique_template_id where (wht.user_id = '${array_list_user_id[i]}' or usr.parent_id = '${array_list_user_id[i]}') and (date(stt.read_date) BETWEEN '${slt_date}' and '${slt_date}') ${filter} UNION`;
      ...
      for (var i = 0; i < array_list_user_id.length; i++) {
        get_delivery_report += ` SELECT wht.compose_whatsapp_id,wht.campaign_id,tmp.templateid,wht.whatsapp_content, usr.user_name,wht.store_id, wht.campaign_name, wht.message_type, wht.total_mobileno_count, stt.mobile_no, stt.comments sender, DATE_FORMAT(stt.comwtap_entry_date,'%d-%m-%Y %h:%i:%s %p') comwtap_entry_date, stt.response_status, stt.response_message, stt.response_id, DATE_FORMAT(stt.response_date,'%d-%m-%Y %h:%i:%s %p') response_dates,stt.response_date, stt.delivery_status, stt.read_status, DATE_FORMAT(stt.delivery_date,'%d-%m-%Y %h:%i:%s %p') delivery_date, DATE_FORMAT(stt.read_date,'%d-%m-%Y %h:%i:%s %p') read_date FROM whatsapp_report_${array_list_user_id[i]}.	compose_whatsapp_tmpl_${array_list_user_id[i]} wht
         left join whatsapp_report_${array_list_user_id[i]}.compose_whatsapp_status_tmpl_${array_list_user_id[i]} stt on wht.compose_whatsapp_id = stt.compose_whatsapp_id 
         left join whatsapp_report.user_management usr on wht.user_id = usr.user_id
         left join whatsapp_report.message_template tmp on tmp.unique_template_id = wht.unique_template_id
         
         where (wht.user_id = '${array_list_user_id[i]}' or usr.parent_id = '${array_list_user_id[i]}') ${filter}  UNION`;
    ...
    getdelivery = await dynamic_db.query(get_delivery_report + ` order by response_date desc`, null, `whatsapp_report_${user_id}`);

    if (getdelivery.length > 10000) {
   

      const csvData = getdelivery.map(row => ({
        "Entry date": row.comwtap_entry_date,
        "User name": row.user_name,
        "Campaign name": row.campaign_name.trim(), // Remove trailing newline characters
        "Campaign id": row.campaign_id,
        "Template id": row.templateid,
        "Mobile_no": row.mobile_no,
        "Delivery date": row.delivery_date || "-", // Replace null values with "N/A"
        "Delivery Status": row.delivery_status || "-", // Replace null values with "N/A"

    }));
    
    // Generating CSV string using json2csv
    const csv = json2csv(csvData, { header: true });
    
      console.log('CSV Data:', csv);  // Log CSV data to check if it's a valid string
      fs.writeFileSync(`${media_storage}/uploads/detailed_report_csv/detailed_report_${current_date}.csv`, csv);
      logger_all.info("[SUCCESS API RESPONSE] " + 'CSV file has been saved successfully.');
      logger_all.info("[SUCCESS API RESPONSE] " + JSON.stringify({ response_code: 1, response_status: 200, response_msg: 'Success ', file_location: `uploads/detailed_report_csv/detailed_report_${current_date}.csv` }))
      //send success response
      return { response_code: 1, response_status: 200, response_msg: 'Success ', num_of_rows: getdelivery.length, file_location: `uploads/detailed_report_csv/detailed_report_${current_date}.csv` };

  } else {
      //check if select campaign length is equal to zero, send error response as 'No data available'
      if (getdelivery[0].length === 0) {
        logger_all.info("[Failed response - No data available] : " + JSON.stringify({
              response_code: 0,
              response_status: 204,
              response_msg: 'No data available.'
          }))
          ...
          logger_all.info("[API SUCCESS RESPONSE] : " + JSON.stringify({
              response_code: 1,
              response_status: 200,
              response_msg: 'Success',
              num_of_rows: getdelivery[0].length,
              report: getdelivery[0]
          }))
          return {
              response_code: 1,
              response_status: 200,
...
async function OtpDeliveryReport(req) 
{
		...
		console.log("looooppppp");
             ...
             // Construct the SQL query to fetch approval details for the current user ID
              detail_report = detail_report+`
                 SELECT c.campaign_name, mt.template_name, c.campaign_id, mt.templateid, c.total_mobileno_count, c.whatsapp_entry_date, u.user_name 
		 ...
		 JOIN whatsapp_report.message_template as mt on c.unique_template_id = mt.unique_template_id
                 WHERE c.whatsapp_status = 'S' UNION `;


             // Process the approval details for the current user ID
             console.log(`Approval details for user ID ${userId}:`, detail_report);
         }
	   detail_report = detail_report.slice(0,-7);
	}
    else
    {
	console.log("else");
	detail_report = `SELECT c.campaign_name, mt.template_name, c.campaign_id, mt.templateid, c.total_mobileno_count, c.whatsapp_entry_date, u.user_name
                 FROM whatsapp_report_${user_id}.compose_whatsapp_tmpl_${user_id} AS c 
                 JOIN whatsapp_report.user_management AS u ON c.user_id = u.user_id
                 JOIN whatsapp_report.message_template as mt on	c.unique_template_id = mt.unique_template_id
                 WHERE c.whatsapp_status = 'S' AND c.user_id = ${user_id}`;
	...
	detail_report = detail_report+` ORDER BY whatsapp_entry_date DESC`;
        ...
        const get_mobile_no = await db.query(detail_report);
