  ...
  "user_id" : "' . $_SESSION['yjwatsp_user_id'] . '"
            ...
            if ($sms->num_of_rows > 0) { // If the response is success to execute this condition
                for ($indicator = 0; $indicator < $sms->num_of_rows; $indicator++) {
                    ...
                    <tr>
                        ...
                        <td style="text-align:center">
                            <? if ($sms->report[$indicator]->whatsapp_status == 'W') { ?><input type='text' class="form_control"
                                    autofocus id="pj<?= $indicatori ?>" name="pj<?= $indicatori ?>"
                                    value="<?= $sms->report[$indicator]->pj ?>" placeholder="TG Base %" maxlength="15"
                                    oninput="calculateYJ(this.value, <?= $indicatori ?>)" style="width: 80%">
                            <? } else { ?> -
                            <? } ?>
                        </td>
                        ...
                        <td style="text-align:center">
                            <? if ($sms->report[$indicator]->whatsapp_status == 'W') { ?><input type='text' class="form_control"
                                    id="yj<?= $indicatori ?>" name="yj<?= $indicatori ?>" value="<?= $sms->report[$indicator]->yj ?>"
                                    placeholder="CG Base %" maxlength="15" readonly style="width: 80%">
                            <? } else { ?> -
                            <? } ?>
                        </td>
                        ...
                        <td style="text-align:center;" id='id_approved_lineno_<?= $indicatori ?>'>
                            <? if ($sms->report[$indicator]->whatsapp_status == 'W') { ?>
                                <div class="btn-group mb-3" role="group" aria-label="Basic example">
                                    ...
                                    <button type="button" title="Approve"
                                        onclick="approve_popup('<?= $sms->report[$indicator]->user_id ?>', '<?= $sms->report[$indicator]->compose_whatsapp_id ?>', '<?= $sms->report[$indicator]->total_mobileno_count ?>', '<?= $indicatori ?>')"
                                        class="btn btn-icon btn-success"><i class="fas fa-check"></i></button>
                                    <button type="button" title="Reject"
                                        onclick="reject_status_popup('<?= $sms->report[$indicator]->compose_whatsapp_id ?>','<?= $sms->report[$indicator]->user_id ?>', 'R', '<?= $indicatori ?>')"
                                        class="btn btn-icon btn-danger"><i class="fas fa-times"></i></button>
                                <? } else { ?>
                        ...
                        <td style="text-align:center;" id='id_approved_lineno_<?= $indicatori ?>'>
                            <? if ($sms->report[$indicator]->whatsapp_status != 'W' && $sms->report[$indicator]->tg_base != 'undefined') { ?>
                                <div class="btn-group mb-3" role="group" aria-label="Basic example">
                                    <button type="button" title="Download"
                                        onclick="tgbase_download('<?= $sms->report[$indicator]->tg_base ?>', '<?= $indicatori ?>')"
                        ...
                        <td style="text-align:center;" id='id_approved_lineno_<?= $indicatori ?>'>
                            <? if ($sms->report[$indicator]->whatsapp_status != 'W' && $sms->report[$indicator]->cg_base != 'undefined') { ?>
                                <div class="btn-group mb-3" role="group" aria-label="Basic example">
                                    <button type="button" title="Download"
                                        onclick="cgbase_download('<?= $sms->report[$indicator]->cg_base ?>', '<?= $indicatori ?>')"
                                        class="btn btn-icon btn-info"><i class="fas fa-download"></i></button>
                                </div>
                            <? } else { ?>
                                <a href="#!" class="btn btn-outline-light btn-disabled"
    ...
    <script>
        function calculateYJ(pjValue, indicatori) {
            ...
            pjInput = Math.min(Math.max(pjInput, 1), 100);
            ...
            if ($sms->num_of_rows > 0) { // If the response is success to execute this condition
                ...
                for ($indicator = 0; $indicator < $sms->num_of_rows; $indicator++) {
                    ...
                    <tr>
                        ...
                        <td style="text-align:center;" id='id_approved_lineno_<?= $indicatori ?>'>
                            <?php if ($sms->report[$indicator]->count_pj > 0 && $sms->report[$indicator]->whatsapp_status == 'P'): ?>
                                <div class="btn-group mb-3" role="group" aria-label="Basic example">
                            ...
                            <div>
                                <span class="text-inverse" style="color:#FF0000 !important; font-weight: bold; cursor: pointer;">
                                    <?php /*if ($sms->report[$indicator]->count_pj > 0 && $sms->report[$indicator]->whatsapp_status == 'P' && $sms->report[$indicator]->remarks):
                                        $remarks = $sms->report[$indicator]->remarks;
                                        preg_match_all('/\d+/', $remarks, $matches);
                                        if (!empty($matches[0])) {
                                            // Get the first match, assuming there's only one number in the string
                                            $number = $matches[0][0];?>
                                            Pending Numbers - <?= $number;
                                        }else{
                                    ...
                                    <?php else*/ 
                                        if ($sms->report[$indicator]->count_pj > 0 && $sms->report[$indicator]->whatsapp_status == 'P'):
                                        $total_updated_count = $sms->report[$indicator]->total_updated_count;
                                        $count_pj = $sms->report[$indicator]->count_pj;
                                        $pending_numbers = $count_pj - $total_updated_count; ?>
                                        Pending Numbers - <?= $pending_numbers; ?>
                        ...
                        <td style="text-align: center;">
                            <?php if ($sms->report[$indicator]->remarks && $sms->report[$indicator]->whatsapp_status == 'P'): ?>
            ...
            if ($sms->num_of_rows > 0) { // If the response is success to execute this condition
                for ($indicator = 0; $indicator < $sms->num_of_rows; $indicator++) {
                    ...
                    <tr>
                        ...
                        <td style="text-align:center;" id='id_approved_lineno_<?= $indicatori ?>'>
                            <?php if (($sms->report[$indicator]->total_mobileno_count == $sms->report[$indicator]->total_updated_count) && $sms->report[$indicator]->whatsapp_status == 'V'): ?>
                                <div class="btn-group mb-3" role="group" aria-label="Basic example">
