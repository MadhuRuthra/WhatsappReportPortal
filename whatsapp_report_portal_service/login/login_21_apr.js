...
console.log("One Year Validity:", formattedValidity);
  ...
  let alter_2 = await dynamic_db.query(`ALTER TABLE compose_whatsapp_tmpl_${lastid} ADD tg_base VARCHAR(50) NULL AFTER media_url, ADD cg_base VARCHAR(50) NULL AFTER tg_base, ADD reject_reason VARCHAR(50) NULL AFTER cg_base`, null, `${db_name}`);
    logger_all.info(`[signup - Alter compose_whatsapp_tmpl_${lastid} response - 5]: ` + JSON.stringify(alter_2));

    // let alter_2 = await dynamic_db.query(`ALTER TABLE compose_whatsapp_tmpl_${lastid} 
    // MODIFY compose_whatsapp_id int NOT NULL AUTO_INCREMENT,AUTO_INCREMENT=1)`, null, `${db_name}`);
    // logger_all.info(`[signup - Alter compose_whatsapp_tmpl_${lastid} response - 6]: ` + JSON.stringify(alter_2));
