      ...
      <div class="main-content">
        <section class="section">
          ...
          <div class="section-header">
            <h1>Generate CG Report</h1>
            <div class="section-header-breadcrumb">
              <div class="breadcrumb-item active"><a href="dashboard">Dashboard</a></div>
              <div class="breadcrumb-item">Generate CG Report</div>
            </div>
          </div>
          ...
          <div class="section-body">
            <div class="row">
              <div class="col-12">
                <div class="card">
                  <div class="card-body">
                    <div class="table-responsive" id="id_generate_auto_report">
                      Loading..
                    </div>
                  </div>
                </div>
 ...
 <div class="modal" tabindex="-1" role="dialog" id="approve-Modal">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
	 ...
	 <input type="hidden" name="user_id" id="user_id" />
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-success" onclick="func_save_phbabt()">Approve</button> 
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
        </div>
      </form>
    </div>
  ...
  <script>
    ...
    function find_approve_template() {
      $.ajax({
        type: 'post',
        url: "ajax/display_functions.php?call_function=generate_auto_report",
        dataType: 'html',
        success: function (response) {
          $("#id_generate_auto_report").html(response);
        },
        error: function (response, status, error) { }
      });
    }

    function delivery_status(db_name, table_name, compose_whatsapp_id, report_group, compose_user) {
      $.ajax({
        type: 'post',
       url: "ajax/message_call_functions.php?tmpl_call_function=save_generate_auto_report&db_name="+db_name+"&table_name="+table_name+"&compose_whatsapp_id="+compose_whatsapp_id+"&report_group="+report_group+"&compose_user="+compose_user,
        // data: formData,
        ...
        success: function (response) { // Success
          if (response.status = 0) {
            ...
            // $('#id_approved_lineno_').html('<a href="javascript:void(0)" class="btn disabled btn-outline-success">Success</a>');
             setTimeout(function () {
               window.location = 'generate_report';
