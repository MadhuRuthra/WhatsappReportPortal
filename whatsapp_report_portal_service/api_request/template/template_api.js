...
async function getTemplate(req) {
  try {
    ...
    if (user_type == 4) { //if the user type is '4' the process are executed.and to get the template details to the available user.
      logger_all.info("[select query request] : " + `SELECT distinct tmp.unique_template_id template_id,tmp.template_name,tmp.media_type,lng.language_code, tmp.body_variable_count,tmp.template_message,tmp.templateid  FROM message_template tmp left join whatsapp_config con on tmp.whatsapp_config_id = con.whatspp_config_id left join master_language lng on lng.language_id = tmp.language_id left join user_management usr on con.user_id = usr.user_id left join user_management crt on tmp.created_user = crt.user_id where con.user_id = '${user_id}' and tmp.template_status = 'Y' ORDER BY tmp.template_id DESC`)
