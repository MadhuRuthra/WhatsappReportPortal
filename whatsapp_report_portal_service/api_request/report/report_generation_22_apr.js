...
async function report_update(req) {
    ...
    try {
        ...
        if (select_summary_results[0].generate_status == 'N') {
            ...
            // var update_summary = `UPDATE whatsapp_report.user_summary_report SET total_waiting = 0,total_process = 0,total_failed = ${failed_count},total_read = ${read_count},total_delivered = ${delivery_count},total_success = ${success_count},sum_end_date = CURRENT_TIMESTAMP WHERE com_msg_id = '${compose_whatsapp_id}' and user_id = '${user_id}'`;
            ...
            console.log(`UPDATE whatsapp_report.master_compose_whatsapp SET updated_cgbase = '${totalcg_count}',total_updated_count = ${totalcg_count} WHERE compose_whatsapp_id = ? and user_id = ?`[compose_whatsapp_id, user_id]);

            var update_compose = await db.query(`UPDATE whatsapp_report.master_compose_whatsapp SET updated_cgbase = '${totalcg_count}',total_updated_count =  '${totalcg_count}' WHERE compose_whatsapp_id = ? and user_id = ?`, [compose_whatsapp_id, user_id]);
